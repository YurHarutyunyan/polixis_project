...
Notes:
- The first search for a given term does a live scrape and can take several minutes (by default it parses up to 100 matching companies, each requiring its own detail/officers/PSC page fetches — see the comment in `CompanySearchService.search()` at `src/main/java/org/example/service/CompanySearchService.java:73` if you want to lower that number for faster results).
